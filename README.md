...
## Codex Adaptation Notes (2026-05-28)

This fork has been validated for local Codex usage on Windows. Per the local Codex workspace rule, keep this clone in a dedicated project folder:

```text
C:\Users\MOgicBOOK\Documents\Codex\Github\a-stock-data-codex
```

### Local Codex Skill Layout

Install the main skill into Codex:

```powershell
New-Item -ItemType Directory -Force "$env:USERPROFILE\.codex\skills\a-stock-data"
Copy-Item .\SKILL.md "$env:USERPROFILE\.codex\skills\a-stock-data\SKILL.md" -Force
```

Use an isolated Python environment for this skill. `mootdx` pins `httpx<0.26`, while Codex/MCP tooling may require newer `httpx`; installing everything globally can break unrelated tooling.

```powershell
python -m venv "$env:USERPROFILE\.codex\skills\a-stock-data\.venv"
& "$env:USERPROFILE\.codex\skills\a-stock-data\.venv\Scripts\python.exe" -m pip install -r .\codex\requirements-codex.txt
```

### Quick Codex Helper

`codex/a_stock_basic.py` is a small no-key helper for common lookups:

```powershell
& "$env:USERPROFILE\.codex\skills\a-stock-data\.venv\Scripts\python.exe" .\codex\a_stock_basic.py quote 600519 000858
& "$env:USERPROFILE\.codex\skills\a-stock-data\.venv\Scripts\python.exe" .\codex\a_stock_basic.py info 600519
& "$env:USERPROFILE\.codex\skills\a-stock-data\.venv\Scripts\python.exe" .\codex\a_stock_basic.py industry --top 10
```

Validated on 2026-05-28:

- Tencent quote returned live quote, PE, PB, market cap, limit-up and limit-down fields for `600519` and `000858`.
- Eastmoney industry ranking returned 100 industry rows.
- Eastmoney `push2` stock info can be sensitive to local proxy/DNS state. The helper tries the system network configuration first and falls back to direct requests.

### iwencai SkillHub / announcement-search

This repo vendors the exact CLI and skill files used during the Windows Codex setup:

```text
codex\skillhub\iwencai-skillhub-cli\
codex\skillhub\announcement-search\
codex\install_iwencai_skillhub_windows.ps1
codex\patch_announcement_search.py
```

The original official package URL is:

```powershell
Invoke-WebRequest -Uri "https://www.iwencai.com/skillhub/static/0.0.4/iwencai-skillhub-cli.zip" -OutFile iwencai-skillhub-cli.zip
Expand-Archive iwencai-skillhub-cli.zip -DestinationPath iwencai-skillhub-cli -Force
```

On Windows, the official outer script may look for `aime-install.sh`, while the downloaded zip currently contains `iwencai-install.sh`. The shell wrapper generated by the installer also calls `python3`, which may resolve to the Windows Store stub. From this repo, use the vendored installer instead:

```powershell
.\codex\install_iwencai_skillhub_windows.ps1
```

The installer copies the vendored CLI to `%USERPROFILE%\.iwencai-skillhub`, creates `%USERPROFILE%\.local\bin\iwencai-skillhub-cli.cmd`, copies `announcement-search` into `%USERPROFILE%\.codex\skills`, and applies the local CLI response-shape patch.

You can also install/refresh `announcement-search` directly from SkillHub after the wrapper exists:

```powershell
& "$env:USERPROFILE\.local\bin\iwencai-skillhub-cli.cmd" --dir "$env:USERPROFILE\.codex\skills" install announcement-search --force
python .\codex\patch_announcement_search.py
```
After installation, verify the CLI:

```powershell
& "$env:USERPROFILE\.local\bin\iwencai-skillhub-cli.cmd" --version
```

Required environment variables, with the API key kept out of git:

```powershell
$env:IWENCAI_BASE_URL = "https://openapi.iwencai.com"
$env:IWENCAI_API_KEY = "<your iwencai SkillHub API key>"
```

For persistence, add the same lines to `$PROFILE.CurrentUserAllHosts`, or set user-level environment variables with `[Environment]::SetEnvironmentVariable(...)`.

Validated on 2026-05-28:

- `announcement-search` installed to `%USERPROFILE%\.codex\skills\announcement-search`.
- A real OpenAPI request for `贵州茅台 公告` succeeded with `200`.
- The installed `announcement-search` CLI expected `search.search()` to return a tuple, but the implementation returns a dict. Run `python .\codex\patch_announcement_search.py` after installing the skill to patch `scripts/__main__.py` locally.
- Its `requirements.txt` lists Python standard library modules such as `json`, `csv`, `datetime`, `os-sys`, and `sys` as pip requirements. Do not install those blindly; `requests`, `pandas`, and `python-dotenv` are the relevant third-party dependencies.

End-to-end smoke test:

```powershell
$env:IWENCAI_BASE_URL = [Environment]::GetEnvironmentVariable("IWENCAI_BASE_URL", "User")
$env:IWENCAI_API_KEY = [Environment]::GetEnvironmentVariable("IWENCAI_API_KEY", "User")
Push-Location "$env:USERPROFILE\.codex\skills\announcement-search\scripts"
& "$env:USERPROFILE\.codex\skills\a-stock-data\.venv\Scripts\python.exe" .\__main__.py "贵州茅台 公告" --limit 1
Pop-Location
```
